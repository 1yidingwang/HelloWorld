...
#### __init__ vs __new__
```shell
1. __init__ 方法为初始化方法, __new__方法才是真正的构造函数，是一个类方法。
__new__()方法用于创建实例，类实例化之前会首先调用，它是class的方法，是个静态方法。
而__init__()方法用户初始化实例，该方法用在实例对象创建后被调用，它是实例对象的方法，用于设置类实例对象的一些初始值。
2. _new__方法先于__init__方法执行
3. 如果类中同时出现了__init__()方法和__new__()方法，则先调用__new__()方法后调用__init__()方法。__new__()方法是创建实例的第一步，
执行完了需要返回创建的类的实例，否则则报错，无法执行__init__()方法。其中，__init__()方法将不返回任何信息。
4. __new__ 方法会返回 类的一个实例，通常情况下是使用 super().__new__()这样的方式；
5.__new__ 至少要有一个参数 cls，代表要实例化的类； __new__ 必须要有返回值，返回实例化出来的实例；
```